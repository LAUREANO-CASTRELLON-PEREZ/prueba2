...
btnsave.addEventListener("click",(event)=>{
    const miStorage = window.localStorage;
    const activitysEx = JSON.parse(miStorage.getItem('actividadestercercorte'));
    ...
    if(activitysEx!=null){
      miStorage.removeItem('actividadestercercorte');
    ...
    miStorage.setItem("actividadestercercorte", JSON.stringify(activitysEx));
    ...
    location.reload()
      ...
      miStorage.setItem("actividadestercercorte", JSON.stringify(actividades));
      if(porcentajeviejo==null){
        miStorage.setItem('porcentaje',porcentaje);
      }else{
        miStorage.setItem('porcentaje', parseInt(porcentajeviejo) + parseInt(porcentaje));
...
const cargaractividades = () =>{
    const miStorage = window.localStorage;
    const actividades = JSON.parse(miStorage.getItem('actividadestercercorte'));
    const listaactividades = document.getElementById("lista_activity_corte_3");
    ...
    let porcentajehtml = document.getElementById('porcentajetercercorte');

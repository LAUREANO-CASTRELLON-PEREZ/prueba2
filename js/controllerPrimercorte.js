...
btnsave.addEventListener("click",(event)=>{
    const miStorage = window.localStorage;
    const activitysEx = JSON.parse(miStorage.getItem('actividadesprimercorte'));
    ...
    if(activitysEx!=null){
      miStorage.removeItem('actividadesprimercorte');
    ...
    miStorage.setItem("actividadesprimercorte", JSON.stringify(activitysEx));
    ...
    location.reload()
      ...
      miStorage.setItem("actividadesprimercorte", JSON.stringify(actividades));
      if(porcentajeviejo==null){
        miStorage.setItem('porcentaje',porcentaje);
      }else{
        miStorage.setItem('porcentaje', parseInt(porcentajeviejo) + parseInt(porcentaje));
...
const cargaractividades = () =>{
    const miStorage = window.localStorage;
    const actividades = JSON.parse(miStorage.getItem('actividadesprimercorte'));
    const listaactividades = document.getElementById("lista_activity_corte_1");
    ...
    let porcentajehtml = document.getElementById('porcentajeprimercorte');
